...
{
    SLinkedList* pNode = pLList->pPrevNode; // Points to the last node the original list.
    ...
    // Join the first node with the last node of the list...
    pCurNode->pNextNode = pNewList;
    pNewList->pPrevNode = pCurNode;
...
void main()
{
    ...
    // Join the first node with the last node of the list...
    pNode->pNextNode = pLList;
    pLList->pPrevNode = pNode;
    ...
    do // Exit control loop is the best for this situation.
    {
        // Display the values.
    ...
    } while(pRNode != pRepList); // Appropriate since both the nodes must end together.
}
